...
import com.blockchain.dao.SpotAnalysisMapper;
...
/**
 * @author weiwenchang
 * @description 针对表【spot_analysis(现货分析表)】的数据库操作Service实现
 * @createDate 2024-09-11 21:54:00
 */
@Service
@RequiredArgsConstructor
public class SpotAnalysisServiceImpl extends ServiceImpl<SpotAnalysisMapper, SpotAnalysis>
        implements SpotAnalysisService {
    ...
    public void parseData(SpotQueryParam spotQueryParam) {
        ...
        List<SpotAnalysis> spotAnalysisList = new ArrayList<>();
        for (int i = 0; i < jsonArray.size(); i++) {
            ...
            SpotAnalysis spotAnalysis = SpotAnalysis.builder()
                    ...
                    .build();
            spotAnalysisList.add(spotAnalysis);
            //System.out.println(spotAnalysis);
        }
        System.out.println(spotAnalysisList.size());

        macdCalculator.calculateAndFill(spotAnalysisList);
        super.saveBatch(spotAnalysisList);
    }
